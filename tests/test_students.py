...
class StudentViewTests(StudentTestCase):
    ...
    def test_admin_allows_student_crud_forms_and_validates_input(self):
        enrollment, link = self.enroll(), self.link()
        ...
        for model, record in (("student", self.student), ("guardian", self.guardian), ("studentguardian", link), ("enrollment", enrollment)):
            ...
            self.assertEqual(self.client.post(detail, {}).status_code, 200)
            self.assertEqual(self.client.get(reverse(f"admin:students_{model}_add")).status_code, 200)
            self.assertEqual(self.client.get(reverse(f"admin:students_{model}_delete", args=[record.pk])).status_code, 200)
